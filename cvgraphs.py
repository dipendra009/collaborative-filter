...
if __name__ == "__main__":
	files = ["6f3.txt","6f4.txt"]#, "6ee4.txt", "6ee8.txt", "6ee16.txt", "6ee32.txt"]#, "6c3","6c4"]
	fd = []
	td = []
	datap = []
	i = 0
	for f in files:
		...
		data = fo.read()
		#print data
		data = data.split("averageerrors: ")
		#print data[1]
		data = [ float(x.strip().replace('[', '').replace(']','').strip()) for x in data[1].split(',')]
		datap.append(data)
		#fd, td = datap[0], datap[1]
		#print fd
		#print td
	...
	data = datap[0]
	plt.subplot(2,1,1)	
	plt.title("Histograms for item_based and user_based")
	
	plt.hist(data)
	plt.xlabel("Errors Values")
	plt.ylabel("Error Frequency")
	plt.xlim(min(data), max(data))
	data = datap[1]
	plt.subplot(2,1,2)	
	#plt.title("Histograms for Manhattan Distance")
	plt.hist(data)
	plt.xlabel("Value of Errors")
	plt.ylabel("Number of Errors")
	plt.xlim(min(data), max(data))
	 
	#plt.show()
	plt.savefig("6fgraphs.pdf")
	plt.close()
	...
	#print datap[2]
	#print datap[3]	
	stat12 = stats.ttest_ind(datap[0], datap[1], equal_var=False)
	print stat12
	#stat2 = stats.ttest_ind(datap[2],datap[3], equal_var = False)
	print "means: ", sum(datap[0])/len(datap[0]), sum(datap[1])/len(datap[1])
